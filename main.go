...
func initDb(dbUrl string) apiConfig {
...
func mountRouter(apiCfg apiConfig, oauthSrv *server.Server) *chi.Mux {
...
func startupOauth(dbUrl string, apiCfg apiConfig) *server.Server {
...
func main() {
	...
	apiCfg := initDb(dbUrl)
	oauthSrv := startupOauth(dbUrl, apiCfg)
	router := mountRouter(apiCfg, oauthSrv)
